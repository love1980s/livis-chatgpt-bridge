...
import {
  attachCodexNativeDaemon,
  type CodexNativeDaemonAttachDependencies,
  type CodexNativeDaemonAttachOptions,
  type CodexNativeDaemonAttachment,
} from "./native-daemon.ts";
...
export interface CodexNativeSessionHarnessOptions {
  transport: Omit<CodexNativeDaemonAttachOptions, "onNotification">;
...
export interface CodexNativeSessionHarnessDependencies {
  ...
  attachDependencies?: CodexNativeDaemonAttachDependencies;
}
...
/**
 * 把 proxy 构造时固定的 notification callback 延迟绑定到 coordinator 的确切 client epoch。
 *
 * initialize 到 coordinator ready 之间没有合法 active attempt；这段窗口的 notification 只计数并
 * 丢弃，不保存内容。bind 后的每条事件都携带绑定时 epoch，旧 proxy 不能借迟到事件命中新代际。
 */
...
async function closeAttachmentAfterStartFailure(
  attachment: CodexNativeDaemonAttachment,
  ...
  try {
    ...
    throw new AggregateError(
      ...
      "Codex native harness 启动失败且 proxy client 收口未确认",
    );
  }
  throw primaryError;
}
...
/**
 * transport attach 与持久 session coordinator 的受控组合 harness。
 *
 * 本模块不进入 daemon/index/config，不读取账号状态，不管理原生 daemon，也不声明生产可用；它只供
 * fake 端点离线组合验证。唯一拥有的外部资源是 relay 自己启动的 proxy client。
 */
export class CodexNativeSessionHarness {
  ...
  private constructor(
    private readonly attachment: CodexNativeDaemonAttachment,
  ...
  static async start(
    ...
    const attachment = await attachCodexNativeDaemon({
      ...options.transport,
