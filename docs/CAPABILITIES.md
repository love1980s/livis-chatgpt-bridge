...
无状态纯文本 adapter、doctor、JobStore attempt、原子切换和真实 LiViS App 单条文本闭环已有
证据，仍为 `operator-only`；错误态、取消/超时/崩溃和长时运行尚未闭合。同一 daemon 在线多
后端路由仍保持 `unsupported`，不能因三个 adapter 分别存在而提升状态。
