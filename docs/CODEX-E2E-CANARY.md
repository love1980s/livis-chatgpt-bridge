...
### 1.1 2026-07-24 完整编码态脱敏结果

测试绑定提交 `b10155f`、同一固定 wire contract、Codex 0.145.0、显式 custom Responses
provider、固定模型 `gpt-5.6-sol`、Bun 1.3.14 与唯一获准来源设备。私有脱敏回执 ID 为
`codex-coding-20260724-5cd0bb95295d810c`；公开文档不保存 endpoint、Agent/node/thread/job
ID、token、原始请求或完整 rollout。

本轮使用全新的专用 state，完成 profile schema v1→v2 原子迁移、fresh LiViS Device Flow、
`upstream check` 与 `doctor --online`。按操作者明确要求，它复用本机 Codex CLI、custom
provider、模型和同一 API key，但不共享日常 `~/.codex` 的 thread、rollout、插件、memory
或 workspace。现有 `OPENAI_API_KEY` 只经标准输入登录专用 `CODEX_HOME`；没有复制、symlink
或 hardlink 日常 `auth.json`，两个凭据文件均为不同 inode 的 `0600` 单链接普通文件。

操作者只发送一次第 5.2 节的固定请求，真实回执为：

- 唯一 nonce job 为 `Succeeded`、`run_generation=1`，attempt ledger 精确为
  `reserved → accepted → succeeded`；outbox 为 `Delivered`、一次 delivery attempt、
  `retry_count=0`，Relay 已 ACK；
- rollout 有 4 个真实 tool call 及 4 个对应 output：工具链读回、创建目录、`apply_patch`
  写入三个文件、`bun test`；approval、user-input request、unknown item 均为 0；
- permission profile 只有 workspace 写根，`/opt/homebrew/bin` 只读，网络关闭，审批策略为
  `never`；工具读回 `bun=/opt/homebrew/bin/bun`、版本 1.3.14；
- `package.json`、`src/add.ts`、`tests/add.test.ts` 均为 workspace 内 `0644`、单链接普通文件，
  无第三方依赖；Agent 测试与操作者独立复跑均为 `3 pass / 0 fail`；
- Bun 在 sandbox 内成功测试时隐式探测 workspace 父级 state 目录，系统明确返回
  `PermissionDenied`；tool 命令未显式指向外部路径，没有成功读取或写入 state，退出码仍为
  0。该信息作为隔离生效的观察项保留，不能删日志后声称 stderr 全空；
- SQLite 结果正文与唯一 terminal final 精确一致；checkpoint `0→1`，四个 active 字段清空，
  `recovery_required=0`，backend backlog 与 quarantine 均为 0；
- 操作者确认 LiViS App 只显示一个 final 气泡，正文与数据库结果精确一致。

成功目录、SQLite 与 rollout 继续保留作证。回执采集时 Codex daemon 以前台方式运行；本轮
没有安装或修改常驻 LaunchAgent，因此它证明固定现场组合的完整编码闭环，不证明重启自恢复
或无人值守生产运行。
...
## 5. 完整编码态工具 canary（2026-07-24 已取得一次回执）
...
- 第 5 节已在 2026-07-24 的固定 macOS/Codex/Bun/provider/model/profile 组合取得一次真实
  回执；这不表示其他工具链、版本、设备或部署也已经通过。
