...
这里复用已有 v1 `failed`：daemon 只在当前 `jobId + leaseId` 上把 `Dispatching/Running` 持久化为 `Failed`、写入通用失败答复 outbox，随后才回 `result_stored`；bridge 必须在收到该 job 的同一 websocket generation 上等待 durable ACK，超时或 lease 不匹配时只关闭原连接，不能把未确认拒绝当成已完成，也不能误关期间建立的新连接。job offer 一到达就建立 tombstone，使门禁前的 cancel 不会误派内部 `/stop`；bridge 只有在能够证明 `handle_message()` 从未开始时，才在既有 v1 `cancelled` 上附加 `executionDisposition=not_started`。daemon 对该证明按当前 lease 原子结算 `Cancelled`，不建立 quarantine；字段缺失或值未知时保持失败关闭。普通输入通过门禁后还会在首次 `await` 前保留 Hermes session admission reservation，防止相邻 job 同时穿过尚未建立的 `_active_sessions` 窗口。该变化不新增 connector v2 或 JobStore schema 迁移。daemon 为 Relay cancel 生成的内部 `/stop` 由 `_handle_cancel()` 直接派发，不经过远程文本门禁；它仍保持既有 `CancelUnknown` 与 session quarantine 语义。
...
runtime。精确提交 `56a1d77` 随后又以隔离 API-key/custom provider 完成一个成功固定短答
turn，实际零工具、workspace 未变、session/SQLite/进程组均收口，临时凭据也已删除。
回执仍不覆盖 Linux、请求层工具 schema、endpoint 内部 HTTP 次数、逃逸进程组后代、
LiViS App 回显或未来版本，Codex 后端仍不能宣称生产上线。详细矩阵见
[Codex app-server 执行后端](CODEX-APPSERVER.md#当前上线门禁)。

2026-07-23，精确提交 `896091b` 又完成一次隔离的 Codex 人在环 S4：LiViS App、Relay、
daemon、Codex app-server/custom Responses、durable outbox 与 App 回显形成高层功能闭环。
主 canary 只有一个规范 job，状态为 `Succeeded`、`run_generation=1`，attempt ledger 为
`reserved → accepted → succeeded`，outbox 为 `Delivered` 且只有一次 delivery attempt；
操作者确认 App 只显示一个正文完全一致的回复气泡。操作者自己发送的另外两条测试消息
也分别形成独立的 `Succeeded/Delivered` job；同一 rollout 共三轮，实际工具、审批、
用户输入请求和 unknown item 都为 0。单气泡和单次 delivery attempt 只说明本次观察没有
重复，不能把端到端语义升级为 exactly-once；三轮零工具也只说明没有实际 tool item，
不能证明发往 provider 的请求 payload 不含工具 schema，更不能证明 provider 内部只有一次
HTTP 请求。完整脱敏事实与验收边界见
[Codex 完整 LiViS 人在环 canary](CODEX-E2E-CANARY.md)。

本次功能闭环通过不等于凭据收口完成：LiViS IDaaS revoke 实际返回 HTTP 404，daemon
按失败关闭规则保留本地 refresh token 与隔离 state。因此远端撤销和 canary cleanup 仍被
...
- IDaaS revoke 只有返回 2xx 才允许删除本地 refresh token；网络失败或远端拒绝时保留本地凭据并明确失败，供操作者重试。2026-07-23 的 Codex 人在环 S4 收口实际得到 HTTP 404，因此功能闭环虽已通过，远端撤销与隔离 state cleanup 仍被阻塞。
