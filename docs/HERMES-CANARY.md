...
先按[运行手册的 macOS LaunchAgent 章节](OPERATIONS.md#macos-launchagent)完成静态部署、双服务启动和 JobStore v6 备份/升级边界。随后记录：
...
当前 daemon 首次打开旧数据库时还可能执行 JobStore v6 迁移；因此升级前必须停掉 Relay/Hermes 并完整备份 state directory，不能把数据库迁移成功等同于消息验收。若 v4 仍有待派发 job，还必须先确认并显式声明其原始 backend，不能借迁移切换 provider。
