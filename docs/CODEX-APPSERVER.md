...
网络探针使用 host 侧原始 TCP listener/connect 正向 control；sandbox 内先确认
`/usr/bin/perl -MSocket` 可用，再对同一 loopback endpoint 执行原始 `connect`。只有退出
码为 77、输出 `CONNECTED=0` 且 errno 精确为 `EPERM(1)` 或 `EACCES(13)`，同时 listener
仍只有一次宿主正向连接，才裁决工具网络权限被拒绝；普通超时、`ECONNREFUSED`、其他
errno、Perl/Socket 缺失或真实 TCP 命中全部失败关闭。
...
`perlSocketProbeAvailable=true` 与 `toolNetworkPermissionDenied=true`，hardlink/工具网络
负向结论才成立。workspace 与外部牺牲文件若不在同一文件系统，或目标平台没有
`/bin/ln`、`/usr/bin/perl`/Socket，入口会按“无法裁决”失败，不会把 `EXDEV`、普通超时
或命令缺失当成 sandbox 通过。
...
5. 执行 `bun run src/index.ts session release '<sessionKey>'`；命令会先在 connector socket
   路径取得 daemon offline guard，再由同一 SQLite 事务退役存在 recovery 的 backend
   session，或没有 active/recovery 证据但已 quarantine 的 idle session；
...
尚不可释放的 active/recovery 证据时仍失败关闭。该操作不会删除旧 rollout、workspace、
job/outbox，也不会撤销文件、命令或外部系统副作用。这是显式 session reset：下次启动
会按当时 command、账号、模型和安全配置创建并绑定新 thread。JSON 中的
...
撤销，也不再输出不精确的 `nextStartCreatesNewThread` 推断。

## 当前上线门禁

当前实现与 fake 回归要求只有显式选择 `local` environment 后，approval、permission
profile、workspace-only writable roots、无工具网络以及 `/tmp` 隔离才会同时命中预期；
空 environment 数组会退化为 `readOnly`。较早候选的真实 `thread/start` 安全回读曾覆盖
基础读取隔离，但当前增强组合门禁还没有完整真实回执。
...
`CODEX_HOME`、`OPENAI_*`、`LIVIS_*` 未进入 sandbox 子进程环境。该历史回执只覆盖当时的
macOS/CLI/config 组合，不可当作当前增强 canary 的完整结果，也不可外推到 Linux 或未来
Codex 版本。
...
因此当前实现与历史回执已经覆盖“空账号协议接线、基础读取隔离、零 turn 恢复与高风险
feature 冻结”，但增强的 hardlink/command/TCP 组合门禁尚未闭合，整体仍只能作为受控
开发功能，不能宣称生产上线。继续集成至少还要：

- 用本地 fake Responses endpoint 截获工具定义，确认只有已审核的 core/workspace 工具；
- 先让当前 macOS/Codex 0.145.0 的扩展 canary 在不扩大 agent 读取面的前提下完成
  Perl/Socket（或等价无外部运行时探针）正向 control，再取得牺牲文件 hardlink、原始
  TCP errno 与 command identity 的完整真实回执；之后仍须在 Linux、未来 Codex 版本和
  filesystem profile 变更后重跑；
...
`bun run check` 通过 342 个 Bun 测试和 22 个 Hermes pytest。本轮真实 Codex 0.145.0
零模型 turn smoke 得到 `sentModelTurn=false`、`zeroTurnMaterialized=true` 和
`zeroTurnResumeVerified=true`；较早的非临时基础读取 canary 曾确认 workspace 可读写、
专用 `CODEX_HOME` 与宿主 HOME/TMPDIR 读写被拒绝，agent HOME/TMPDIR 可写，敏感环境
变量不可见。当前新增的 hardlink、command identity 与原始 TCP errno 组合 canary 只运行到
Perl/Socket 正向 control，尚未取得完整真实回执：sandbox 内 `/usr/bin/perl -MSocket` 因系统
`libperl.dylib` 读取被拒绝而以 exit 134 失败关闭。失败发生在 `thread/start` 与模型 turn
之前，命令没有产出包含全部 required fields 的成功 JSON，因此不能把 fake 测试或旧读取
回执写成新增门禁已通过。

已取得的回执只证明当时 macOS 组合上的无模型协议、持久化和基础读取隔离边界。smoke 使用未登录的
