...
因此，下面的人工恢复顺序只用于 active turn 不确定、metadata/checkpoint/tail 漂移、
quarantine、关闭无法确认或恢复预算耗尽；满足 fence 且已自动恢复成功的 idle 退出不应
执行 `session release`。
...
`bun run check` 通过 317 个 Bun 测试和 22 个 Hermes pytest。本轮真实 Codex 0.145.0
