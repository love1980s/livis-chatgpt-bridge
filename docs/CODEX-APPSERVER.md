...
登录”。Codex 0.145.0 对 API key 与 ChatGPT 账号会正常返回 `true`；daemon 以
`account` 是否为受支持的对象判断是否已有账号，并继续由真实 turn 裁决凭据是否有效。
`account=null` 无论该布尔值为何都不能通过生产 backend 启动。
...
Codex 0.145.0 在 provider 失败后会把权威 `turn/completed` 标为 `failed`，同时让
`thread/read.status.type` 保持为 `systemError`；下一次 `turn/start` 才清除该 thread 状态。
daemon 只在 terminal 通知、thread ID、turn ID、failed tail、rollout 和 active fence
全部精确一致时接受这一形态并结算 `Failed`。同一 app-server client epoch 中的下一次
dispatch 还必须持有该 terminal 实际观察到的内存 marker，并与 SQLite checkpoint 和当前
ID/status/count/hash 完全一致；marker 不跨 client/recovery 持久化。fresh start、daemon
重启和 idle recovery 仍要求严格 `idle`。`completed/interrupted + systemError`、未记录
tail、后来无关的 `systemError` 或任一漂移继续按执行不确定失败关闭。
...
`bun run check` 通过 374 个 Bun 测试和 22 个 Hermes pytest。本轮真实 Codex 0.145.0
...
随后一次单 turn、禁止工具的获授权 canary 已取得 provider thread/turn 接受回执。为这次
...
本地 token 计数为 0。该调用确实发出，因此没有自动重试；它只暴露 provider 错误终态与
该副本的凭据有效性缺口，不能证明专用登录凭据状态，也不能替代成功模型 turn 或 LiViS
回显验收。修复后的 backend 会把同类结果结算为脱敏 `Failed`，原子隔离 session 并停止
继续使用该凭据；本轮失败证据目录保持原样，未自动清理。

该回执只证明当前 macOS/Codex 0.145.0 组合上的无模型协议、持久化和安全边界。smoke 使用未登录的
