...
随后一次单 turn、禁止工具的获授权 canary 已取得 app-server `turn/start` 提交回执和
...
做窄语义归一化，并继续以 raw turns hash 检测漂移；fake 回归已通过。没有再次发送真实 turn，仍需专用登录
凭据和新的明确授权才能复验真实 `Failed + Pending outbox + credential quarantine` 收口。
