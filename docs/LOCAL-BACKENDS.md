...
本文定义 `livis-relayd` 调用本机 Hermes、Codex 和 Claude 的目标架构。当前仍只有 Hermes connector 完成实现与实网 canary；Codex、Claude 处于 `contract-only`，不得据此文档宣称已经接入或可用于生产。
...
| Codex 认证 | Codex 原生运行时 | 调用 app-server，不读取 token | `backend_auth_unavailable` |
...
LiViS OAuth 与本地推理后端认证是两个独立安全域。backend adapter 不得导入 `IdaasClient` 或 `SecretStore`，daemon 也不得把 LiViS token 交给本地后端。

## 3. 强制认证边界

daemon 和 backend adapter 必须遵守以下规则：

1. 不读取、解析、复制或监听 Codex、Claude、Hermes 的凭据文件、Keychain 条目或认证环境变量。
...
这些边界由 [`src/backend/contract.ts`](../src/backend/contract.ts)、[`tests/local_backend_contract.test.ts`](../tests/local_backend_contract.test.ts) 和 [`tests/auth_boundary.test.ts`](../tests/auth_boundary.test.ts) 共同约束。
...
- 只调用经版本门禁的 `codex app-server`。
- 使用隔离工作区和最小权限，不读取 `~/.codex/auth.json`。
...
- 增加显式本地路由配置和 session affinity。
- 对三种 backend 独立归档版本、命令、结果与失败证据，不归档 token。
- 验证后端切换不会改写任一认证存储。
- 真实 canary 完成前，机器可读能力契约不得宣称 Codex/Claude 可用。
