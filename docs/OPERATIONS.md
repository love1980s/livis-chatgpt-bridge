...
提交 `65f00c1` 上的全新获授权 canary 又只发送了一个 turn；隔离的本地 ChatGPT OAuth 副本
被 provider 以 structured `unauthorized` 拒绝。该轮真实验证了 job `Failed`、outbox
...
证据。因此该轮只判定 `FIX_GO_ONLY`，不能作为账号或模型功能成功证明。`Pending` outbox
...
`session release`；任何新的真实调用都必须换全新目录并重新取得明确授权。`account/read`
只证明账号对象存在，不能证明凭据当前有效，今后不得用复制日常凭据代替专用登录。
