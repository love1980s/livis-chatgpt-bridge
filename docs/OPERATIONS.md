...
Linux 将权限检查替换为 `stat -c '%a'`。上述命令启动后只从标准输入接收 API key，不得写入
argv、环境变量、config、日志或 shell history。首次登录必须显式固定 `file`，后续 daemon 配置也会固定同一值，
...
若操作者明确要求复用本机同一 custom provider 的 API key，必须先在不输出 endpoint/key
的前提下确认日常 Codex 与 relay config 的 provider 类型、endpoint 和 model 精确一致；
只允许把现有 `OPENAI_API_KEY` 通过内存管道送入上述专用登录命令。该做法只复用凭据，不得
共享日常 `~/.codex` 的 config、thread、rollout、插件、memory、workspace 或其他状态；也不
得复制、symlink 或 hardlink 整个 `auth.json`。登录后必须读回两边均为 `0600`、单链接、
不同 inode 的普通文件。因为两套 state 共用同一 provider key，服务端轮换会同时影响两边；
任何 key/provider 变更仍必须停止旧 daemon 并创建全新 state，不能原地覆盖后继续旧 thread。

不要把 `account/read.requiresOpenaiAuth=true` 误判为尚未登录或 OAuth 模式；它只表示当前
provider 依赖 OpenAI 认证。生产门禁要求 `account.type` 精确为 `apiKey`；空账号、
`chatgpt`、`amazonBedrock` 和未知类型都会在 permission profile 与 thread RPC 前拒绝，
每次 dispatch 还会在 `turn/start` 前重新回读账号并与内存/SQLite 锚点核对。运行中认证模式
漂移会隔离 session、关闭 backend 且不发送 turn；API key 的实际有效性仍由获授权真实模型
请求裁决。
...
custom endpoint 会收到 API key、prompt、会话上下文和工具结果；
`acknowledgeApiKeyTransmission=true` 同时确认这四类数据出口，而不只是字段名中的 API key。
`baseUrl` 必须是无 userinfo、query、fragment 的 HTTPS URL；首期不支持额外 header、query、
`env_key`、`experimental_bearer_token` 或命令认证。daemon 会固定
`wire_api=responses`、`requires_openai_auth=true`、request/SSE retry 均为 `0` 且关闭
...
精确提交 `b10155f` 于 2026-07-24 又完成一次真实完整编码态 canary。该轮在全新专用 state
完成 profile schema v2 迁移、fresh LiViS Device Flow、online doctor 与同 provider/model/key
的专用 API-key 登录；只复用本机 CLI/provider/model/key，不共享日常 Codex 会话状态。
`toolchainReadRoots=["/opt/homebrew/bin"]` 只读暴露 Bun 1.3.14，workspace 是唯一写根，工具
网络关闭且 approval policy 为 `never`。

获授权用户只发送一次固定 nonce 编码请求。唯一 job 达到 `Succeeded/Delivered`、
`run_generation=1`、`reserved → accepted → succeeded`、单次 delivery attempt；rollout
真实记录工具链读回、目录创建、`apply_patch` 三个文件和 `bun test`，4 个 tool call/output、
零 approval/user-input/unknown。Agent 与操作者独立测试均为 `3 pass / 0 fail`，用户确认 App
只显示一个与数据库正文精确一致的 final 气泡；checkpoint `0→1`，active/recovery/backlog/
quarantine 全清。Bun 的内部父目录探测被 sandbox 对 state root 以 `PermissionDenied` 拒绝，
没有成功读取外部数据；该观察项保留在回执中，不能写成 stderr 全空。私有脱敏回执 ID 为
`codex-coding-20260724-5cd0bb95295d810c`，详见
[Codex 完整 LiViS 人在环 canary](CODEX-E2E-CANARY.md#11-2026-07-24-完整编码态脱敏结果)。

本轮只以前台 daemon 取得回执，没有安装或修改 Codex 常驻 LaunchAgent；因此结论为固定现场
组合的 `E2E_CODING_GO`，不包含重启自恢复、无人值守常驻、Linux/cgroup 或长期刷新验证。
...
该轮目录 `.livis-relay-real-canary-20260723-e71363f-r2` 是只读证据：不得复用、执行
`session release`、改库或清理。
...
提交 `65f00c1` 上的全新获授权 canary 又只发送了一个 turn；该轮误选的本机非默认凭据副本
被 app-server 与 SQLite 识别为 `account_type=chatgpt`，不属于支持的 API-key 路径，并被
provider 以 structured `unauthorized` 拒绝。该轮真实验证了 job `Failed`、outbox
`Pending`、ledger `reserved/accepted/failed`、checkpoint `failed/1`、active clear、
`recovery_required=false`、单条 credential quarantine、SQLite 完整性和零敏感模式命中，
不再出现旧轮次的 `Interrupted + recovery_required=true`。rollout 没有 assistant、工具或
token-count 记录；源凭据文件没有被修改。报告顶层 `ok=false`，因为一次性 harness 只接受
上一轮写死的 `401 invalid_api_key` 脱敏文案，而本轮落库为 `Codex provider 认证失败`，且
报告生成瞬间 `lsof` 无法裁决；报告已确认进程组关闭，事后 `lsof` 为零，但不能倒推瞬时
证据。因此该轮只判定通用失败结算的 `FIX_GO_ONLY`，不能作为 API-key 凭据或模型功能成功
证明；当前生产门禁会在 thread 前拒绝同类账号。`Pending` outbox
只是通用失败结果等待 Relay 投递，不表示模型任务仍在运行；`recovery_required=false`
只表示没有遗留不明确的 active turn，session quarantine 仍会阻止自动重试，不能据此
视为 backend ready。
...
API key 登录并重新取得明确授权。
`account/read.account.type=apiKey` 只证明返回了 API-key 类型账号对象，不能证明该 key 当前
有效，今后不得用复制日常凭据代替专用登录。

这里的旧证据特指可另行审阅、且不属于上述 r2/r3 保留目录的状态。OpenAI/custom 切换、
custom endpoint 变更和 API key 轮换都不得对旧 state 执行 `session release` 后原地继续；
固定流程为：

1. 停止旧 daemon，确认 app-server 与工具子进程退出，备份旧 state 并记录其 provider；
2. 按 provider 的管理面撤销旧 key，并在旧专用 `CODEX_HOME` 本地 logout；外部撤销与本地
   清除必须分别确认，任一步失败都不得声称旧 key 已失效；
3. 创建全新 `stateDir`、config、数据库、workspace 和专用 `CODEX_HOME`，不要复制旧
   `auth.json`、rollout、SQLite 或 session；
4. 通过标准输入把新 API key 登录到新专用 `CODEX_HOME`，配置唯一 provider；
5. 先运行无秘密 strict-config/account/provider probe，再以全新 state 做一次获授权真实
   canary，最后才允许接入唯一 LiViS 设备。

原因是当前 `jobs.target_backend` 只绑定 `codex`，没有绑定其 OpenAI/custom 子 provider；
同时 `account/read` 无法区分两把 API key。旧 Codex backlog 不能跨 provider/key 自动执行。
...
custom provider 候选必须从独立 relay config 读取 provider/model，不能把 endpoint 或 key
作为额外 CLI 参数。无模型 strict-config 探针示例：

```bash
bun run smoke:codex:app-server -- \
  --command /opt/homebrew/bin/codex \
  --config /绝对路径/custom-canary-config.json
```

该入口仍固定 `sentModelTurn=false`，只能验证 strict config、账号类型、provider/model
回读和零-turn materialize/resume；它不裁决 API key 有效性，也不等于真实 Responses turn。

全新目录的 `backendStartReady=false` 表示当前没有可供生产 backend 使用的 API-key 账号，
不是协议 smoke 失败；该入口允许 `account=null` 仅用于零模型协议诊断，非空且非 `apiKey`
的账号会在创建 thread 前拒绝。
